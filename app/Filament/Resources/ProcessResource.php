...
class ProcessResource extends Resource
{
    ...
    protected static ?string $pluralLabel = 'Processes';
    protected static ?string $label = 'Process';
            ...
            ->schema([
                ...
                Forms\Components\Select::make('parent')
                    ...
                    ->options(function (callable $get){
                        if($get('name') != NULL) {
                            return Process::where('name', '!=', $get('name'))->pluck('name', 'id');
                        }
                        return Process::all()->pluck('name', 'id');
                    }),
        ...
        return $table
            ...
            ->filters([
                ...
                Tables\Filters\MultiSelectFilter::make('territories')
                    ->label(__('Territories'))
                    ->relationship('territories', 'name', fn(Builder $query) => $query->has('processes')),
                Tables\Filters\MultiSelectFilter::make('departments')
                ->label(__('Departments'))
                    ->relationship('departments', 'name', fn(Builder $query, $livewire) => $query->has('processes')),
                Tables\Filters\MultiSelectFilter::make('positions')
                    ->label(__('Positions'))
                    ->relationship('positions', 'name', fn(Builder $query) => $query->has('processes')),
                Tables\Filters\MultiSelectFilter::make('workers')
                    ->label(__('Workers'))
                    ->relationship('workers', 'last_name', fn(Builder $query) => $query->has('processes')),
