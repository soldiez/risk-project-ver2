            ...
            ->schema([
                ...
                ->label(__('Parent'))
                    ->options(function (callable $get){
                        if($get('name') != NULL) {
                            return Service::where('name', '!=', $get('name'))->pluck('name', 'id');
                        }
        ...
        return $table
            ...
            ->filters([
                ...
                Tables\Filters\MultiSelectFilter::make('territories')
                    ->label(__('Territories'))
                    ->relationship('territories', 'name', fn(Builder $query) => $query->has('services')),
                Tables\Filters\MultiSelectFilter::make('departments')
                ->label(__('Departments'))
                    ->relationship('departments', 'name', fn(Builder $query, $livewire) => $query->has('services')),
                Tables\Filters\MultiSelectFilter::make('positions')
                    ->label(__('Positions'))
                    ->relationship('positions', 'name', fn(Builder $query) => $query->has('services')),
                Tables\Filters\MultiSelectFilter::make('workers')
                    ->label(__('Workers'))
                    ->relationship('workers', 'last_name', fn(Builder $query) => $query->has('services')),
